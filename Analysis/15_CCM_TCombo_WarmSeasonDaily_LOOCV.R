...
#load data
load("CCM_TCombo_WarmSeasonDaily_SCB3.Rdata")
...
write.csv(bestE, "WarmDaily_bestE_TComboSCB3.csv", row.names = FALSE)
...
compute_ccm_web_1 <- function(in_file = "CCM_TCombo_WarmSeasonDaily_SCB3.Rdata", 
                              best_E_file = "WarmDaily_bestE_TComboSCB3.csv",
                              ...
                              chunk)
{
  ...
  run_ccm <- function(i)
  {
    ...
    ccm_tp_zero <- ccm(temp_df, lib = lib, pred = pred, lib_sizes = seq(1, 2205, by = 245), #MAKING CHANGES HERE!
    ...
    return(data_frame(from = ccm_params$from[i], 
                      to = ccm_params$to[i], 
                      best_E = best_E, 
                      ccm_tp_zero = list(ccm_means(ccm_tp_zero)), 
  ...
  for (i in 1:n_tibble){
    ...
    to = ccm_results[[i]]$to
    actual_rho = tail(ccm_results[[i]]$ccm_tp_zero[[1]]$rho, 1) 
...
compute_ccm_web_1(chunk = 1:5, out_file = "TCombo_WarmDaily_ccm_webs_1.Rdata")
#fuck yeah! that worked!

compute_ccm_web_1(chunk = 6:10, out_file = "TCombo_WarmDaily_ccm_webs_2.Rdata")

compute_ccm_web_1(chunk = 11:15, out_file = "TCombo_WarmDaily_ccm_webs_3.Rdata")

compute_ccm_web_1(chunk = 16:20, out_file = "TCombo_WarmDaily_ccm_webs_4.Rdata")

compute_ccm_web_1(chunk = 21:25, out_file = "TCombo_WarmDaily_ccm_webs_5.Rdata")

compute_ccm_web_1(chunk = 26:30, out_file = "TCombo_WarmDaily_ccm_webs_6.Rdata")
...
png(file = "ConvergencePlots_Warm.png",
...
save.image("TCombo_WarmDaily_ccm_webs_SCB3.RData")
...
png(file = "SignifancePlots_Warm.png",
    width = 1000, height = 750)
warm_significance
dev.off()
